...
// Optimization: We track the LAST rendered position to avoid repainting the DOM 
// if the mouse hasn't moved enough to justify a new frame.
let lastRenderedCoreX = -1;
let lastRenderedCoreY = -1;
let lastRenderedOrbitX = -1;
let lastRenderedOrbitY = -1;
...
function animateCursor() {
  ...
  // OPTIMIZATION: Only update the DOM if the values have actually changed by more than 0.1px.
  // This completely stops CPU burn when the mouse is sitting still.
  if (Math.abs(coreX - lastRenderedCoreX) > 0.1 || Math.abs(coreY - lastRenderedCoreY) > 0.1) {
      if (cursorCore) cursorCore.style.transform = `translate3d(calc(${coreX}px - 50%), calc(${coreY}px - 50%), 0)`;
      lastRenderedCoreX = coreX;
      lastRenderedCoreY = coreY;
  }

  if (Math.abs(orbitX - lastRenderedOrbitX) > 0.1 || Math.abs(orbitY - lastRenderedOrbitY) > 0.1) {
      if (cursorOrbitWrapper) cursorOrbitWrapper.style.transform = `translate3d(calc(${orbitX}px - 50%), calc(${orbitY}px - 50%), 0)`;
      lastRenderedOrbitX = orbitX;
      lastRenderedOrbitY = orbitY;
  }
